...
(defun gm/math-setup ()
  ...
  (setq gm/math-snippets (append gm/math-global-snippets gm/math-local-snippets))
...
should expand."
  ...
  (let* ((key (read-string "Snippet key: "))
    ...
    (dump-vars-to-file '(gm/math-global-snippets) gm/math-snippet-loc)
    (gm/math-setup)))
#+END_SRC
...
(defun gm/math-frac (&optional env)
  ...
  (cond
   ...
   ((string-match "[[:alnum:]!]" (char-to-string (char-before)))
    (gm/math-frac-helper
     (concat
      (regexp-opt gm/math-frac-extras)
      "?\\( \\|{}\\)?"
      "[\\]?[[:alnum:]_^!]*")
     env))
