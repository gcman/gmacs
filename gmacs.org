...
faster than disabling them within Emacs, but it offers less flexibility. For example, if Emacs fails to start up properly, you are possibly left staring at a
...
(use-package whole-line-or-region
  ...
  :bind (:map org-mode-map
              ([remap org-kill] . 'whole-line-or-region-kill-region)
              ([remap org-yank] . 'whole-line-or-region-yank-handler)))
#+END_SRC
...
(defun gm/math-frac (&optional env)
  ...
  (cond
   ...
   ((and (eq (char-before) 125)
         (not (equal "({}" (gm/last-n-chars 3)))) ;; match closing curly braces
