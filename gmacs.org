...
#+END_SRC
Spellchecking should not occur inside things like proerty drawers and code
...
#+END_SRC
Automatically spellcheck the buffer using =flyspell=.
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
**** Use Math Macros
I have a file with macros for math. I want this to be automatically included in
every LaTeX file exported from Org Mode. The optional third argument makes sure
this package is also loaded when compiling LaTeX image previews, such as
equation previews.
#+BEGIN_SRC emacs-lisp :noweb-ref org-settings
