...
#+END_SRC
We change the default packages used to reflect the necessities of running
=xelatex=. We also use the =memoir= class instead of the =article= class.
#+BEGIN_SRC emacs-lisp
(defun gm/auto-tex-parameters ()
...
(add-hook 'org-export-latex-after-initial-vars-hook 'gm/auto-tex-parameters)
#+END_SRC
**** Export Classes
***** Tufte-LaTeX
#+BEGIN_SRC emacs-lisp
