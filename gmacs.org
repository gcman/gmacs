...
#+BEGIN_SRC emacs-lisp
(bind-key "M-9" #'backward-sexp emacs-lisp-mode-map)
(bind-key "M-0" #'forward-sexp emacs-lisp-mode-map)
#+END_SRC
** Packages
*** Helpful
...
with font-lock by hooking into Org Mode's font-lock.

We set up the maps here.
#+BEGIN_SRC emacs-lisp
(require 'elpy)

(defvar gm/src-block-keymaps
      `(("python" . ,(let ((map (make-composed-keymap
                                 `(,elpy-mode-map ,python-mode-map)
                                 org-mode-map)))
        ...
        ("emacs-lisp" . ,(let ((map (make-composed-keymap
                           ...
                           map))))
#+END_SRC
Then we define the function that applies the keymap to each block.
#+BEGIN_SRC emacs-lisp
...
(defun gm/math-number-subscript ()
  (interactive)
  (cond ((string-match "[a-zA-Z]" (gm/last-n-chars 1)) (insert "_"))
