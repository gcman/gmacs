...
#+END_SRC
*** Attachment
I have started to use Org Mode to manage my files, with the attach
functionality.
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
**** Add Images to Notes Live :ATTACH:
:PROPERTIES:
:Attachments: 20190331_182318.jpg 20190331_164501.jpg 20190331_164434.mp4 20190331_164428.jpg
:ID:       43b5539c-435a-42cc-934d-abbe9f141c3d
:END:
...
image link, and display it.
#+BEGIN_SRC emacs-lisp
(defun gm/insert-live-image ()
  (interactive)
  (let (file-list file-list-sorted start-file start-file-full)
    (setq file-list
    ...
    (insert (org-make-link-string (format "file:%s" (expand-file-name start-file (org-attach-dir nil)))))
...
(emms-playing-time 1)
(bind-keys ("s-j e" . emms)
           ("s-j b" . emms-smart-browse)
           ("s-j r" . emms-player-mpd-update-all-reset-cache))
#+END_SRC
...
(bind-key "s-j c" #'mpd/start-music-daemon)
#+END_SRC
...
(bind-key "s-j k" #'mpd/kill-music-daemon)
#+END_SRC
*** Update MPD Database
#+BEGIN_SRC emacs-lisp
...
(bind-key "s-j u" 'mpd/update-database)
#+END_SRC
* Snippets
