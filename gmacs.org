...
#+BEGIN_SRC emacs-lisp :tangle no noweb-ref deps
(require 'bind-key)
(require 'diminish)
#+END_SRC
** Other Elisp Files
...
has been autosaved. This is unnecessary, so I use [[pragmaticemacs.com/emacs/make-emacs-a-bit-quieter/][this hack]] from Pragmatic Emacs
...
#+BEGIN_SRC emacs-lisp :noweb yes :tangle no :noweb-ref deps
...
performance when checking the entire buffer. I also don't want Emacs telling me
that =flyspell= has started a new process every time it does.
#+BEGIN_SRC emacs-lisp :tangle no :noweb-ref flyspell-settings
...
(defun gm/ispell-word-then-abbrev (p)
  "Call `ispell-word'.  After create an abbrev for the correction
...
#+END_SRC
**** Frog Menu for Corrections
Many forces in Emacs fight for window space, from expanded minibuffers to popup
windows. Both have their disadvantages. The minibuffer is away from the action
and requires you to look to a different place on the screen. Popup windows can
easily cause a jarring change in the way content is displayed on the buffer. The
solution is a [[https://github.com/alphapapa/frog-menu][frog menu]].
...
We use the =flyspell-correct= package to implement such an interface.
#+BEGIN_SRC emacs-lisp :tangle no :noweb-ref flyspell-settings :noweb-ref deps
...
#+END_SRC
**** Capitalize Word
The default behaviour, that pressing =M-c= here =capit|alize= produces =capitAlize= is
silly; I can't think of a situation where I would want to capitalize just the
following character if the cursor is in the middle of a word.
#+BEGIN_SRC emacs-lisp
...
bookmarks (see [[https://karl-voit.at/2014/08/10/bookmarks-with-orgmode/][Karl Volt's method]]), rather than rely on Firefox for that. Also,
I often come across quotes or excerpts that I'd love to file away for later. The
...
wiki]] for the basics. The meat of the following snippet is in the function that
turns the browser selection into HTML (after =&body==). It was sourced from
...
("p" "Org Protocol" entry
 ...
 "* [[%:link][%(square-to-round-brackets \"%:description\")]]\n:PROPERTIES:\n:CREATED: %U\n:END:\n%:initial")
#+END_SRC
...
to fix that up,
#+BEGIN_SRC emacs-lisp :tangle no :noweb-ref org-settings
(defun square-to-round-brackets (string-to-transform)
...
#+BEGIN_SRC js :tangle no
javascript:location.href='org-protocol://capture://n'
#+END_SRC
...
(defun gm/org-preview-fragment (fr)
  ...
  (let ((fr-type (nth 0 fr))
    ...
    (when (and (memq fr-type gm/org-valid-fragment-type)
      ...
      (async-start (symbol-function #'org-toggle-latex-fragment) 'ignore))))
#+END_SRC
...
all composed in plain text with Org Mode. We first need the library that will
convert the Org into HTML.
#+BEGIN_SRC emacs-lisp :tangle no :noweb-ref deps
(require 'org-mime)
#+END_SRC
...
#+END_SRC
Then we get a function to actually send the mail.
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
Now for some customization. If we're sending HTML, we might as well make it
pretty. Let's nicely offset block quotes.
#+BEGIN_SRC emacs-lisp
...
(define-key gm/math-mode-map ";" #'hydra-semicolon-a/body)
#+END_SRC
*** Helper Functions
...
(defvar gm/math-mode-map
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "RET") #'butterfly)
    map))
...
#+BEGIN_SRC emacs-lisp
(--map (eval `(define-key gm/math-mode-map
                ,it (lambda! (gm/math-snippet ,it)) ))
       (delete-dups (--map (s-right 1 (car it)) gm/math-snippets)))
#+END_SRC

#+BEGIN_SRC emacs-lisp
(defun quick-math--add-keymap-to-math-fragments (limit)
  "Add keymaps to src-blocks defined in `scimax-src-block-keymaps'."
  (let ((case-fold-search t))
    (while (re-search-forward (nth 1 (assoc "\\(" org-latex-regexps)) limit t)
      (let ((beg (match-beginning 0))
            (end (match-end 0)))
        (progn
          (add-text-properties
           beg end `(local-map ,gm/math-fragment-map))
          )))))
#+END_SRC
...
(define-minor-mode quick-math-mode
  ...
  (if quick-math-mode
      (progn
        (gm/math-setup)
        (add-hook 'org-font-lock-hook #'quick-math--add-keymap-to-math-fragments t)
        (add-to-list 'font-lock-extra-managed-props 'local-map))
    (remove-hook 'org-font-lock-hook #'quick-math--add-keymap-to-math-fragments))
  (font-lock-fontify-buffer))
#+END_SRC
**** Simpler =texmathp=
...
(defun gm/math-snippet (key)
  (-if-let* ((buff-cand (gm/buff-cand (car gm/longest-snippet) key))
             (completions (--filter
...
(defun gm/bind-number-subscript (n)
  (eval `(define-key gm/math-mode-map
           (number-to-string ,n)
           (gm/math-number-subscript))))
(dotimes (n 10)
...
#+BEGIN_SRC emacs-lisp :tangle no :noweb-ref math-mode
(define-key gm/math-mode-map "/" #'gm/math-frac)
#+END_SRC
