...
#+END_SRC
** Programming
Set up languages loaded for use in Org source blocks. I avoid using
=org-babel-do-load-languages= because that requires loading the entire file =org.el=
on startup, which is quite slow.
#+BEGIN_SRC emacs-lisp :noweb-ref org-settings
...
#+END_SRC
I then set up the default execution commands for my languages.
#+BEGIN_SRC emacs-lisp :noweb-ref org-settings
