...
#+END_SRC
Emacs 25.1 introduced the feature that the quotes used in help buffers to link
to other objects =`these'= display as round quotes =‘these’=. We love some good
typography, so let's enable this feature.
#+BEGIN_SRC emacs-lisp
(setq text-quoting-style t)
#+END_SRC
*** Highlighting
**** Syntax
