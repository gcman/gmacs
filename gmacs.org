...
package on the AUR.
#+BEGIN_SRC emacs-lisp
(defvar gm/unicode-font "DejaVu Sans")
#+END_SRC
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
(defun gm/find-qutebrowser-download-dir (url)
...
(defun gm/find-qutebrowser-download (url)
  (let* ((file (progn (string-match "\\(tmp.*\\.pdf\\)" url)
         ...
         (qutedir (gm/find-qutebrowser-download-dir url))
         (dldir "~/Downloads/")
...
#+BEGIN_SRC emacs-lisp
(require 'org-auctex)
;; (defun gm/math-preview-toggle-subtree ()
;;   (interactive)
;;   (save-restriction
...
;;               ("C-c C-x C-l" . 'gm/math-preview-toggle-subtree)))
#+END_SRC
    ...
    (when-let ((filebase (org-entry-get (point) "EXPORT_FILE_NAME")))
      (let* ((class (org-entry-get (point) "EXPORT_LATEX_CLASS"))
        ...
        (with-temp-file file
          ...
          (replace-regexp-in-region "\\\\emph{Step " "\\\\step{Step "))))))
#+END_SRC
Bind this in Org mode.
#+BEGIN_SRC emacs-lisp :tangle no :noweb-ref org-keys
...
Return the last n characters before the point, or =loc= if provided. Provide less
...
(defvar gm/math-snippet-by-suffix)

(defun gm/math-snippet--bind-keys (keys)
  (dolist (key keys)
    (eval `(define-conditional-key gm/math-mode-map
               ,key (lambda! () (gm/math-snippet-expand ,key))
             (gm/in-math-p)))))

(defun gm/math-snippet--create-suffix-tree (keys)
  (setq gm/math-snippet-by-suffix nil)
  (dolist (key keys)
    (push (cons key '()) gm/math-snippet-by-suffix))
  (dolist (snippet gm/math-snippets)
    (let* ((key (car snippet))
           (last (s-right 1 key))
           (rest (s-left -1 key)))
      (push rest (alist-get last gm/math-snippet-by-suffix nil nil 'equal)))))

(defvar gm/math-unique-snippets nil)
...
(defun gm/math-setup ()
  ...
  (setq gm/longest-snippet
    ...
    (cl-loop for i from 0 to 9
             do (push (number-to-string i) gm/math-unique-suffixes))
    (append (--map (s-right 1 (car it)) gm/math-snippets) gm/math-unique-suffixes)
    (setq gm/math-unique-suffixes (delete-dups gm/math-unique-suffixes))
    (gm/math-snippet--bind-keys gm/math-unique-suffixes)
    (gm/math-snippet--create-suffix-tree gm/math-unique-suffixes))
#+END_SRC
...
(choose function).
#+BEGIN_SRC emacs-lisp
(defun gm/math-snippet-expand-p (key)
  (-if-let* ((buff-cand (gm/buff-cand (car gm/longest-snippet)))
             (completions (-intersection
                           buff-cand
                           (alist-get key gm/math-snippet-by-suffix nil nil 'equal)))
             (snippet-match (--max-by (> (length it) (length other)) completions))
             (snippet (assoc (concat snippet-match key) gm/math-snippets))
             (expand (cadr snippet)))
      snippet))
#+END_SRC
#+BEGIN_SRC emacs-lisp
(defun gm/math-snippet-expand (key)
  (-if-let* ((snippet (gm/math-snippet-expand-p key))
             (expand (cadr snippet)))
      (progn
  ...
  (let ((pt (point)))
    (save-excursion
      ...
      (when (and (string-match-p "\\w" (char-to-string (char-before)))
                 (not (member (thing-at-point 'word 'no-properties) gm/math-frac-non-words)))
        (backward-word)
...
(defun gm/math-frac--backward-match ()
  ...
  (let ((pt (point-min)))
    (while (not (or (eq (point) (point-min))
      ...
      (let ((ptcand (save-excursion (gm/math-frac--backward-word)
                                    (point))))
        (if (eq ptcand (point))
            (goto-char pt)
          (goto-char ptcand))))
...
#+BEGIN_SRC emacs-lisp
(defun gm/math-in-commutative-diagram ()
