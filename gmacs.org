...
#+END_SRC
Sometimes, however, there are other cases
in which we want to reload a buffer. The change have been not in the file's
contents, but in an Emacs setting, such as the colours of the theme. Or the
buffer might not have been representing a file at all. In these cases, it's
usually easiest to revert every open buffer. This might be a problem for you if
you're one of those people who keeps hundreds of buffers open at a time, but I
like to close my buffers liberally with the knowledge that =recentf= will remember
where I've been for me. I obtained the following function from [[https://emacs.stackexchange.com/questions/24459/revert-all-open-buffers-and-ignore-errors][Emacs Stack
Exchange]].
#+BEGIN_SRC emacs-lisp
...
tangle =gmacs.org=. This unfortunately means that Emacs takes a long time to start up (about 6 seconds). To
...
#+BEGIN_SRC emacs-lisp
(defun gm/add-package (&optional package)
  (interactive)
...
Sometimes, we have to fill in a text box in the browser. If only we could use
Emacs for this! But copy-pasting can be annoying. Get the [[https://addons.mozilla.org/en-US/firefox/addon/edit-with-emacs1/][Edit with Emacs]] addon
for Firefox. Ideally, I would use the [[https://addons.mozilla.org/en-US/firefox/addon/ghosttext/][GhostText]] addon (this extension also
exists for Chrome) along with the =atomic-chrome= Emacs package, but I haven't
figured out how to activate that one with the keyboard. I'd need to call a JS
function, which I could do with ShortKeys, but I'm not sure which one to call.
The advantage this has over =edit-server= and Edit with Emacs is that the
GhostText method syncs your edits live between the browser and Emacs, while with
Edit with Emacs, your edits only show up once you "commit" them with =C-c C-c=.
#+BEGIN_SRC emacs-lisp
...
(setq org-refile-targets `((,(expand-file-name "todo.org" org-directory) :maxlevel . 3)
                           ...
                           (,(expand-file-name "reading.org" org-directory) :maxlevel . 3)))
#+END_SRC
*** Refile and Archival
Log refiling.
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
So how do we actually get data from Firefox to Emacs? The [[https://orgmode.org/worg/org-contrib/org-protocol.html][Org Mode wiki]] has the
basics. The meat of the following snippet is in the function that turns the
browser selection into HTML (after =&body==). It was sourced from [[https://stackoverflow.com/questions/5643635/how-to-get-selected-html-text-with-javascript/6668159#6668159][StackOverflow]].
...
*** Export
#+BEGIN_SRC emacs-lisp
(add-to-list 'load-path (expand-file-name "tufte-org-latex/" gmacs-custom-dir))
(defun gm/org-latex-export ()
  ...
  (save-excursion
    ...
    (let* ((class (org-entry-get (point) "EXPORT_LATEX_CLASS"))
           ...
           (filebase (org-entry-get (point) "EXPORT_FILE_NAME"))
      ...
      (if filebase
          (progn
            ...
            (async-start-process (format "latex-export-%s" file)
                                 "latexmk"
                                 (lambda (file) (message (format "Exported %s" file)))
                                 (if (string= cmd "PDF") "-pdf" "-xelatex")
                                 "-interaction=nonstopmode"
                                 file))
        (error "Not in valid export location!")))))
#+END_SRC
Bind this in Org mode.
#+BEGIN_SRC emacs-lisp :tangle no :noweb-ref org-keys
...
**** Command Links
:PROPERTIES:
:header-args: :tangle no :noweb-ref org-settings
:END:
No option.
#+BEGIN_SRC emacs-lisp
...
(use-package visual-fill-column
  ...
  (add-hook 'notmuch-show-mode-hook (lambda () (auto-fill-mode -1))))
#+END_SRC
...
(defun gm/elfeed-toggle-unread ()
  (let ((entry (gm/elfeed-get-entry)))
    (if (member 'unread (elfeed-entry-tags entry)))))
#+END_SRC
*** Hydra
...
(defvar gm/math-snippet-loc (expand-file-name "math-snippets.el" gmacs-emacs-dir))
#+END_SRC
**** Hydras
#+BEGIN_SRC emacs-lisp :tangle no :noweb-ref hydra
  ...
  (when (string-match "[a-zA-Z]" (gm/last-n-chars 1))
    (let ((match
      ...
      (save-excursion
        ...
        (when (string= (gm/last-n-chars 5) "\\vec ")
          (setq match (concat "\\vec " match))))
      match)))
#+END_SRC
  ...
  (let* ((key (read-string "Snippet key: "))
    ...
    (unless (or (s-blank-str? key)
      ...
      (unless (file-exists-p file-cand)
        ...
        (let ((snippet (list key name)))
          (when (s-blank-str? text)
            (setcdr (last snippet) t))
          (push snippet gm/math-snippets))
        (dump-vars-to-file '(gm/math-snippets) gm/math-snippet-loc))
      (gm/math-setup))))
#+END_SRC
...
(defun gm/inkscape-figure ()
  (interactive)
  (let ((default-directory (expand-file-name "img/" default-directory))
        (name (read-string "Figure name: ")))
    (shell-command (format "inkscape-figures create %s" name))
