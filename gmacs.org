...
#+END_SRC
#+BEGIN_SRC emacs-lisp
(defun gm/find-qutebrowser-download (file)
  (let* ((qutedir (expand-file-name (car
                                     (--filter (string-match "qutebrowser-downloads" it)
                                               (directory-files "/tmp/"))) "/tmp"))
         (filecands (split-string file " - "))
         (filenamecand (car filecands))
         (filename (expand-file-name
                    (car (--filter (string-match filenamecand it) (directory-files qutedir)))
                    qutedir)))
    (expand-file-name filename qutedir)))
#+END_SRC
#+BEGIN_SRC emacs-lisp :tangle no :noweb-ref org-settings
(add-to-list
 'org-capture-templates
 '("q" "Org Protocol Qutebrowser pdf" entry
   (file org-default-notes-file)
   "* %^{Title}%(let* ((dir \"~/org/data\") (org-attach-directory dir)) (require 'org-attach) (org-attach-attach (gm/find-qutebrowser-download \"%:description\") nil 'cp) \"\")\n:PROPERTIES:\n:CREATED: %U\n:Author: %^{Author}\n:END:\n%?\n"))
#+END_SRC
*** Org Download
...
should expand."
  ...
  (let* ((key (read-string "Snippet key: "))
         ...
         (snippet (if (s-blank-str? text)
                      (list key name t)
                    (list key text)))
         (snippet-keys (--map (car it) gm/math-snippets))
         (snippet-list (--map (cons (car it) it) gm/math-snippets)))
    ...
    (when (and (member (car snippet) snippet-keys)
      ...
      (setq gm/math-snippets (delete (cdr (assoc key snippet-list)) gm/math-snippets)))
...
(defun gm/math-make-marginnote-float ()
  ...
  (if (org-in-regexp org-bracket-link-regexp 1)
      (let* ((remove (list (match-beginning 0) (match-end 0)))
             (description (org-match-string-no-properties 3))
             (shift-regexp ";[0-9]*ex")
             ...
             (raw (org-element-property :raw-link element)))
        (when (equal path "marginnote")
          (apply 'delete-region remove)
          (yas-expand-snippet (format "[[latex-opt:marginnote][%s;${1:%s}ex]]" (replace-regexp-in-string shift-regexp "" description) shift))))))
#+END_SRC
*** Inkscape Figures
