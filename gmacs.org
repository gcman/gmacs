...
uncompiled =.el= file, a prefix argument does the job. I disable =after-save-hook=
because I set up asynchronous tangling and byte-compilation [[#tangle-async][here]].
#+BEGIN_SRC emacs-lisp
(defun gm/reload (p)
  (interactive "P")
...
for preview inside org-mode. Returns nil at-displayable fragments."
  (let* ((fr (org-element-context))
    ...
    (when (memq fr-type cw/org-valid-fragment-type)
      ...
      (unless (eq (point) (org-element-property :end fr))
        (list fr-type
