...
(dolist (dir (list gmacs-custom-dir
                   gmacs-cache-dir))
...
#+END_SRC
Only check if local packages differ from upstream ones when they are edited in Emacs.
#+BEGIN_SRC emacs-lisp
(setq straight-check-for-modifications 'live)
#+END_SRC
We use =straight.el= to install =use-package=. The variable
=straight-use-package-by-default= is the replacement for
=use-package-always-ensure=. The latter uses =package.el=, which we do not want.
This allows packages to be installed and built on the fly.
#+BEGIN_SRC emacs-lisp
...
(use-package dash :defer 2 :config (dash-enable-font-lock))
...
:END:
I used to have my =init.el= set up so that it would always tangle =gmacs.org=. This
unfortunately means that Emacs takes a long time to start up (about 6 seconds).
To solve this, I tangle this very file on save.
...
(use-package youtube-dl
  ...
  :custom (youtube-dl-directory (concat "~/" gm/youtube-dl-dir)))
#+END_SRC
