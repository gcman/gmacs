    ...
    (when-let ((filebase (org-entry-get (point) "EXPORT_FILE_NAME")))
      (let* ((class (org-entry-get (point) "EXPORT_LATEX_CLASS"))
        ...
        (if (get-process processname)
            (when-let (processbuffer (get-buffer (concat "*" processname "*")))
              (with-current-buffer processbuffer
                (erase-buffer)))
...
(defun gm/play-with-mpv (link &optional title)
  ...
  (let* ((id (nth 1 (s-split "watch\\?v=" link)))
    ...
    (start-process
     ...
     (format "nohup 1>/dev/null 2>/dev/null %s \"%s\"" "mpv" link))))
...
(defun gm/math-setup ()
  ...
  (let ((unique-suffixes
    ...
    (gm/math-snippet--bind-keys unique-suffixes)
    (gm/math-snippet--create-suffix-tree unique-suffixes)))
#+END_SRC
