...
Upon reformatting, each REGEXP is replaced with TO. DOCSTRING simply explains what the regexp does.")
...
(defun gm/in-math-p ()
  (let ((begin (save-excursion
                 ...
                 (point)))
        (end (point))
        inline-open inline-close env-open env-close)
    (defun gm/--in-math-p (begin-delim)
      (concat
       begin-delim
       (s-join "\\|" (--map (concat "\\(" it "\\)") gm/math-environments))
       "[\\*]?}"))
    (setq inline-open (how-many "\\\\(" begin end)
...
(defun gm/math-snippet (key)
  (setq buff-cand (gm/buff-cand gm/buff-cand-max key))
  (setq completions (--filter
                     (member (reverse (car it)) buff-cand)
                     (radix-tree-keys gm/math-suffix-tree key)))
  (setq snippet (--max-by (> (length (car it)) (length (car other))) completions))
  (if snippet
...
(defun gm/math-frac-helper (regexp)
  (let (begin end match)
    (when (looking-back regexp (line-beginning-position) t)
      ...
      (setq match (buffer-substring-no-properties begin end))
