...
(defun my/org-preview-when-leaving-math ()
...
(add-hook 'post-command-hook 'my/org-preview-when-leaving-math t)
#+END_SRC
*** CDLaTeX
#+BEGIN_SRC emacs-lisp
...
(defun my/latex-preview-when-leaving-math ()
 (let ((in-math (texmathp)))
   (cond (in-math
          (setq my/was-inside-math t))
         ((and (not in-math)
...
(add-hook 'post-command-hook 'my/latex-preview-when-leaving-math t)
#+END_SRC
*** Diffing
#+BEGIN_SRC
...
(defhydra hydra-l (:color blue
                          :body-pre (insert "l")
                          :idle 1.0)
  ("e" (if (and (gm/prefix-before-point "le") (not (gm/prefix-before-point "\\le")))
