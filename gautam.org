...
(defun gm/math-insert-frac-match (match)
  (when match
    (progn (insert (concat "\\frac{" match "}{"))
  ...
  (cond ((string-match "[\])]" (char-to-string (char-before)))
         (let (match)
           (progn
             ...
             (backward-sexp)
             (setq match (buffer-substring-no-properties (region-beginning) (region-end)))
...
(defun gm/simplify-math-script (&rest r)
  (when (string-match "[_^]{.}" (gm/last-n-chars 4))
    (save-excursion
      ...
      (backward-char)
      (delete-backward-char 1))))
(advice-add 'yas-next-field-or-maybe-expand-snippet :after #'gm/simplify-math-script)
#+END_SRC
*** Math Symbols
#+BEGIN_SRC emacs-lisp
