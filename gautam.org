...
(add-hook 'org-mode-hook
          (lambda () (dotimes (n 10)
            (eval (macroexpand
                   `(define-conditional-key org-mode-map
                        (number-to-string ,n)
                        (gm/math-subscript (number-to-string ,n)) (texmathp)))))))
#+END_SRC
**** Simplify Super/Subscripts
#+BEGIN_SRC emacs-lisp
(defun gm/enclose-sub-super-script (s)
  (if (and (> (length s) 1) (not (eq (substring s 0 1) "{"))) (concat "{" s "}") s))
